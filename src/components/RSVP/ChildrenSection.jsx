...
const ChildrenSection = ({
  ...
  <div className="mt-6 p-6 bg-white/10 rounded-xl border border-white/20">
    <h4 className={`${language === 'th' ? 'font-krub font-[400]' : 'font-source-serif font-[300] antialiased'} text-xl text-white mb-4`}>{language === 'th' ? "เด็ก" : "Children"}</h4>
    <div className="space-y-3 mb-4">
      <label className="flex items-center gap-3 cursor-pointer" onClick={() => onHasChildrenChange('yes')}>
        ...
        <span className={`text-white/90 ${language === 'th' ? 'font-krub font-[400]' : 'font-source-serif font-[300] antialiased'}`}>{language === 'th' ? "มี" : "Yes"}</span>
      </label>
      <label className="flex items-center gap-3 cursor-pointer" onClick={() => onHasChildrenChange('no')}>
        ...
        <span className={`text-white/90 ${language === 'th' ? 'font-krub font-[400]' : 'font-source-serif font-[300] antialiased'}`}>{language === 'th' ? "ไม่มี" : "No"}</span>
      </label>
    </div>
      ...
      <FadeInExpand className="space-y-4 pt-2">
        <div className="flex flex-col sm:flex-row gap-4 items-stretch w-full min-w-0">
          <div className="w-full sm:w-1/3 flex min-w-0">
            <InputField
              label={language === 'th' ? "ต่ำกว่า 7 ปี" : "Under 7 yrs"}
          ...
          <div className="w-full sm:w-1/3 flex min-w-0">
            <InputField
              label={language === 'th' ? "7 - 12 ปี" : "7 - 12 yrs"}
              labelClassName={language === 'th' ? "font-krub" : "font-source-serif font-[300] antialiased"}
              type="select"
              options={Array.from({ length: 11 }, (_, i) => i)}
              value={children7To12}
          ...
          <div className="w-full sm:w-1/3 flex min-w-0">
            <InputField
              label={language === 'th' ? "12 ปีขึ้นไป" : "12+ yrs"}
              labelClassName={language === 'th' ? "font-krub" : "font-source-serif font-[300] antialiased"}
              type="select"
              options={Array.from({ length: 11 }, (_, i) => i)}
