...
test('ทดสอบสุ่มกรอกฟอร์ม RSVP จำนวน 1 ครั้ง', async ({ page }) => {
  ...
  if (isAttending) {
    ...
    if (waitGroupRate) {
      ...
      // เลือกเข้าพักคนเดียว หรือ แชร์ห้อง
      const isSharing = faker.datatype.boolean();
      ...
      if (isSharing) {
        ...
        if (isShareNotSure) {
          ...
          // กรอกชื่อคนแชร์
          await page.locator('label:has-text("Name:"), label:has-text("ให้ใส่ชื่อ:")').locator('..').locator('input[type="text"]').fill(faker.person.fullName());
        }
      } else {
        // คนเดียว (No/ไม่มี)
        await page.locator('label').filter({ hasText: /^(No|ไม่มี)$/i }).click();
